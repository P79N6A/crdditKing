                      ...
                      <el-row :span="12">
                       ...
                       <el-row :span="12">
                            <el-form-item label="商戶名稱">
                                <el-input v-model="formData.merchant.name" style="width:200px;" v-if="formData.merchant"></el-input>
                            </el-form-item>
                        </el-row>
                        <el-row :span="12">
                          <el-form-item label="優惠類別">
                               <el-select filterable v-model="formData.merchant.discountsTypeId" placeholder="請選擇" @change="discountsTypeChange" style="width:200px;" v-if="formData.merchant">
                                    <el-option
                          ...
                          <el-row :span="12">
                                <el-form-item label="門店" >
                                    <el-row v-for="(a,i) in formData.merchantShop" :key="i" style="margin-bottom:10px;">
                                         <el-input placeholder="請輸入門店名稱" v-model="formData.merchantShop[i]['name']" style="width:200px;" >
                                           <template slot="append">門店名稱</template>
                                        </el-input>
                                         <el-input placeholder="門店地址" v-model="formData.merchantShop[i]['address']" style="width:365px;" >
...
export default {
  ...
  methods: {
    ...
    getMerchantDetail() {
      let loadingInstance = Loading.service({ fullscreen: true });
      this.$get("merchantDetail", this.$route.query.id)
        .then(res => {
          console.log(res)
          this.formData = res;
          this.formData.merchant.saleTime = this.saleTime;
          loadingInstance.close();
        })
        .catch(() => {
          loadingInstance.close();
