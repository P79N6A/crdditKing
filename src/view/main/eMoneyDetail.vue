    ...
    <ui-layout :brs="brs">
        <ui-container>
                <el-form :inline="true" :model="formData" ref="formData" class="demo-form-inline" label-position="center" label-width="130px">
                    <el-col >
                      <el-row :span="12">
                            <el-form-item label="電子貨幣包圖片" prop="url">
                                <el-upload
                        ...
                        <el-row :span="12">
                            <el-form-item label="專輯頁面圖片" prop="banner">
                                <el-upload
                                    class="avatar-uploader"
                                    action="http://47.75.102.93:80/ck/file/upload"
                                    :show-file-list="false"
                        ...
                        <el-row :span="12">
                            <el-form-item label="簡介">
                                <el-input v-model="formData.intro"  type="textarea" :rows="10" style="width:500px;"></el-input>
                            </el-form-item>
                        </el-row>
                        <el-row>        
                            <el-form-item label=" ">
...
export default {
  ...
  methods: {
    getMoneyDetail() {
      ...
      this.$get("eMoneyDetail", this.$route.query.id)
        .then(res => {
          this.formData = res;
   ...
   handleAvatarSuccessUrl(res, file) {
    ...
    toEdit() {
      this.$refs["formData"].validate(valid => {
        if (valid) {
          let loadingInstance = Loading.service({ fullscreen: true });
          this.$post("eMoneySave", this.formData)
            .then(res => {
              this.$message({
                showClose: true,
                message: "編輯電子貨幣包成功",
