                        ...
                        <el-row :span="7">
                         ...
                         <el-row :span="20">
                                <el-form-item label="类别" >
                                    <el-row>
                                        <template>
                                          <el-checkbox v-model="formData.own" label="1">所有</el-checkbox>
                                        </template>
                                    ...
                                    <el-row>
                                        <template>
                                          <el-checkbox v-model="formData.feedBack" label="1">現金回贈</el-checkbox>
                                        </template>
                                    ...
                                    <el-row>
                                        <template>
                                          <el-checkbox v-model="formData.mile" label="1">里數</el-checkbox>
                                        </template>
                                    ...
                                    <el-row>
                                        <template>
                                          <el-checkbox v-model="formData.gift" label="1">禮物</el-checkbox>
                                        </template>
...
export default {
  ...
  methods: {
    getCardDetail() {
      ...
      this.$get("cardDiscountsNewDetail", this.$route.query.id)
        .then(res => {
          this.formData = res;
          if(this.formData.own == 1){
            this.formData.own = true
          }
          if(this.formData.feedBack == 1){
            this.formData.feedBack = true
          }
          if(this.formData.mile == 1){
            this.formData.mile = true
          }
          if(this.formData.gift == 1){
            this.formData.gift = true
          }
          if(this.formData.choice == 1){
            this.formData.choice = true
          }
    ...
    toEdit() {
      this.$refs["formData"].validate(valid => {
        ...
        if(this.formData.ownSort == ""){
          this.formData.ownSort = 0
        }
        if(this.formData.feedBackSort == ""){
          this.formData.feedBackSort = 0
        }
        if(this.formData.mileSort == ""){
          this.formData.mileSort = 0
        }
        if(this.formData.giftSort == ""){
          this.formData.giftSort = 0
        }
        if(this.formData.choice == ""){
          this.formData.choiceSort = 0
        }
        if (valid) {
          let loadingInstance = Loading.service({ fullscreen: true });
